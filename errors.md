...
The following errors occur when something goes wrong while handling the schema of the datasource (e.g. collection) that the SQL query is querying data from.
Often times, these errors occur due to data types being used in an incorrect or invalid way. 

| Error Code                  | Error Description                                                                                                                                                                 | 
|-----------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [Error 1001](#error-1001-)  | A function (e.g., Sin, Abs, Round) was given the incorrect number of arguments.                                                                                                   |  
| [Error 1002](#error-1002-)  | The specified operation (e.g., Sub, And, Substring) was given argument(s) of the incorrect type (e.g., string, int).                                                              | 
...
The following errors occur when something goes wrong while converting the SQL query to MQL, such as incorrect argument counts or failing to find a field reference or datasource.
...
| [Error 3004](#error-3004-)  | The array datasource contains an identifier. Array datasources must be constant.                                                                                            |
| [Error 3005](#error-3005-)  | SELECT DISTINCT is not allowed.                                                                                                                                             |
| [Error 3006](#error-3006-)  | Distinct UNION is not allowed.                                                                                                                                              |
| [Error 3007](#error-3007-)  | A datasource referenced in the SELECT list could not be found.                                                                                                              |
| [Error 3008](#error-3008-)  | A field could not be found in any datasource.                                                                                                                               |
| [Error 3009](#error-3009-)  | A field exists in multiple datasources and is ambiguous.                                                                                                                    |
...
| [Error 3016](#error-3016-)  | A derived datasource has overlapping fields.                                                                                                                                |
...
| [Error 3025](#error-3025-)  | Schema information is insufficient to allow for flattening the datasource.                                                                                                  |
...
## Error Codes Beginning With "1"

### Error 1001:
- **Description:** A function (e.g., Sin, Abs, Round) was given the incorrect number of arguments.
- **Common Causes:** A function was given an extra argument or is missing one. For example, in `SELECT ROUND(a,1,5) FROM foo`
    where `a` is an `int`, ROUND has an extra argument. 
- **Resolution Steps:** The error message will let you know which function has the wrong number of arguments and how much
    it should have. Use this information to correct your query. Corrected example query: `SELECT ROUND(a,1) FROM foo`.

### Error 1002:
- **Description:** The specified operation (e.g., Sub, And, Substring) was given argument(s) of the incorrect type (e.g., string, int).
- **Common Causes:** A field of the wrong type was mistakenly used in the specified function. For example, `SELECT ROUND(str,5) FROM foo`
    where `str` is a string will cause this error. 
- **Resolution Steps:** The error message will let you know which function has the wrong type and what the expected type is.
...
- **Common Causes:** The column you are attempting to aggregate on is either of type `object` or `array` which are not types comparable to themselves. 
    For example, `SELECT * FROM myCol GROUP BY a AGGREGATE MIN(obj) as min` where `obj` is an `object` will throw this error.
- **Resolution Steps:** Only use AGGREGATE functions on columns of the appropriate type. 
    If you want to AGGREGATE on a value within an `array` or `object`, use the UNWIND or FLATTEN datasource keywords.
...
- **Common Causes:** Any use of `COUNT(DISTINCT *)` (e.g., `SELECT COUNT(DISTINCT *) FROM foo`) will not work.
- **Resolution Steps:** Remove any use of `COUNT(DISTINCT *)` from your queries. 

### Error 1005:
- **Description:** The specified comparison operation (e.g., Lte, Between) could not be done due to incomparable types of their operands (e.g. comparing an int to a string).
...
- **Resolution Steps:** The error message will let you know which comparison operation has conflicting types.
...
- **Resolution Steps:** Since this error is likely to arise due to a typo, the error message will suggest similar fields
    that you may have meant to put. However, if nothing similar is found, nothing will be suggested.  
...
- **Common Causes:** Your scalar subquery may return more than 1 document (or row in SQL). For example, `SELECT (SELECT a FROM foo) FROM bar` will cause this error
...
- **Common Causes:** The column you are attempting to sort on is either of type `object` or `array` which are not types comparable to themselves. For example,
    `SELECT * FROM foo ORDER BY obj` where `obj` is an `object` will throw this error.
- **Resolution Steps:** Only sort on columns of the appropriate type.
    If you want to sort on a value within an `array` or `object`, use the UNWIND or FLATTEN datasource keywords.
...
- **Common Causes:** The column you are attempting to group by is either of type `object` or `array` which are not types comparable to themselves. For example,
    `SELECT * FROM foo GROUP BY obj` where `obj` is an `object` will throw this error.
- **Resolution Steps:** Only group by columns of the appropriate type.
    If you want to group by a value within an `array` or `object`, use the UNWIND or FLATTEN datasource keywords.
    Corrected example query: `SELECT * FROM foo ORDER BY a` where `a` is an `int`.
...
- **Common Causes:** You may be looking in the wrong database or have made a type that is causing the collection to not be found. 
    For example, if collection `baz` does not exist, the query `SELECT * FROM baz` will cause this error. 
...
- **Common Causes:** Using extended JSON in queries and expecting it to implicitly cast to the correct data type. For example, someone may write the query 
    `select _id from customers where _id = '{"$oid":"5ca4bbcea2dd94ee58162a6a"}'` because they think the extended JSON string will implicitly cast to an `ObjectID`.
    However, this is not the case. 
- **Resolution Steps:** Don't use Extended JSON format and always explicitly CAST. The error message will try to recommend what you should do (e.g., casting). 
...
- **Common Causes:** Something may have been spelled wrong or put in the wrong order/format. For example, `SELECT ** FROM foo AS f` causes this error due to
    the `**` type.
- **Resolution Steps:** Ensure that everything is spelled correctly and put in the correct order/format. Additionally, this error will suggest a list of
    tokens that you may have meant to put. Corrected example query: `SELECT * FROM foo AS f`.

  
## Error Codes Beginning With "3"
...
### Error 3004:
- **Description:** The array datasource contains references. Array datasources must be constant.
- **Common Causes:** Accessing a field in an array datasource as shown by this query: `SELECT * FROM [{'a': foo.a}] AS arr`.
- **Resolution Steps:** Modify your array datasource to only contain constants. Corrected example query: `SELECT * FROM [{'a': 34}] AS arr`.

### Error 3005:
- **Description:** SELECT DISTINCT is not allowed.
- **Common Causes:** `SELECT DISTINCT` was used in a query. For example, the query `SELECT DISTINCT * from foo` will cause this error.
...
### Error 3007:
- **Description:** A datasource referenced in the SELECT list could not be found.
...
- **Resolution Steps:** Make sure the field is spelled correctly and that you are looking in the correct collection. Additionally, the error message will suggest
    any similar fields you may have meant to write. 

### Error 3009:
- **Description:** A field exists in multiple datasources and is ambiguous.
- **Common Causes:** Two or more fields have the same name in a collection (or some other datasource). For example, assuming `a` is a field in `foo`, 
...
- **Resolution Steps:** Qualify your references (e.g., `<Collection>.<field>` instead of `field`). In the above example, `coll.a` or `coll2.a` would fix this error.

### Error 3010:
- **Description:** The * argument is only valid in the COUNT aggregate function.
- **Common Causes:** You can only do `COUNT(*)`, so doing something like `SUM(*)` will cause this error. For example, the query `SELECT * FROM foo AS arr GROUP BY a AS a AGGREGATE SUM(*) AS gsum`
    will cause this error.
...
- **Common Causes:** Using a scalar function where only an aggregation function can be used. For example, the query `SELECT * FROM foo GROUP BY a AGGREGATE round(a) AS round`
    will cause this error because `round` is a scalar function.
...
- **Common Causes:** Putting anything other than an aggregation or scalar function where an aggregation should be. (Using a scalar function in place of an aggregation will cause a different error: Error 3012).
    For example, the query `SELECT * FROM foo GROUP BY a AGGREGATE COUNT(*) + 7 AS whatever` will cause this error because there is an addition operation along with an aggregation instead of just an aggregation.
...
- **Common Causes:** An aggregation function has more than one argument. For example, the query `SELECT * FROM foo GROUP BY a AGGREGATE min(a,b) AS min` will cause this error
...
- **Common Causes:** Using `DISTINCT` in a scalar function. For example, the query `SELECT ROUND(DISTINCT a,2) FROM foo` will cause this error.
...
### Error 3016:
- **Description:** A derived datasource has overlapping fields.
- **Common Causes:** When multiple datasources in a subquery have fields of the same name, this will happen. For example, assuming collection `bar` and collection `foo` have 
    fields of the same name, then the query `SELECT * FROM (SELECT * FROM foo AS foo, bar AS bar) AS derived` will cause this error.
- **Resolution Steps:** Essentially, you will need to make the selected fields that have the same names have unique names.
...
- **Common Causes:** OUTER JOIN missing a JOIN condition. For example, the query `SELECT * FROM foo AS foo LEFT OUTER JOIN bar` will cause this error because it is missing the 
    `ON <condition>` part.
...
- **Common Causes:** Multiple collections have the same alias. For example, the query `SELECT * FROM foo AS alias, bar AS alias` will cause this error.
...
- **Common Causes:** Selecting more than one field (or column in sql) in a scalar subquery expression. For example, the query `SELECT (SELECT * FROM foo LIMIT 1) FROM bar AS bar`
    will cause this error because the subquery is doing `SELECT *`, so it is selecting multiple fields.
...
### Error 3025:
- **Description:** Schema information is insufficient to allow for flattening the datasource.
- **Common Causes:** Trying to flatten a collection that doesn't have sufficient schema information. For example, assuming you have a collection called
    `noSchemaInfo` that has no schema defined, then the query `SELECT * FROM FLATTEN(noSchemaInfo)` will cause this error. Another common cause is trying
...
- **Common Causes:** Casting to a type that MongoSQL does not support casting for. For example, the query `SELECT CAST(a AS DATE) FROM foo` 
    will cause this error because `DATE` is not a supported target type. 
