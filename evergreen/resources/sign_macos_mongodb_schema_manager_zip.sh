#!/bin/bash
# Uses macnotary to sign and notarize the mac executable `mongodb-schema-manager-macos`.
...
zip -r mongodb-schema-manager.zip mongodb-schema-manager-macos
rm mongodb-schema-manager-macos
...
./darwin_amd64/macnotary \
    --task-comment "Signing the MongoDB Schema Manager" \
    --file "$PWD/mongodb-schema-manager.zip" \
    --mode notarizeAndSign \
    --url https://dev.macos-notary.build.10gen.cc/api \
    --bundleId com.mongodb.mongosql \
    --out-path "$PWD/mongodb-schema-manager-signed.zip"

unzip mongodb-schema-manager-signed.zip
if [ ! -f "mongodb-schema-manager-macos" ]; then
    echo "Error: mongodb-schema-manager-macos does not exist."
    exit 1
fi
