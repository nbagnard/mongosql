...
impl UserError for Error {
    fn code(&self) -> u32 {
        match self {
            ...
            Error::DerivedDatasouceOverlappingKeys(_, _, _) => 3016,
            Error::CannotBeAlgebrized(_) => 3017,
            // Todo: As part of SQL-1604, refactor this to use the wrapped error's code (and user_message down below)
            Error::SchemaChecking(_) => 3018,
    ...
    fn user_message(&self) -> Option<String> {
        match self {
            ...
            Error::DerivedDatasouceOverlappingKeys(_, _, _) => None,
            Error::CannotBeAlgebrized(_) => None,
            Error::SchemaChecking(_) => None,
    ...
    fn technical_message(&self) -> String {
        match self{
            ...
            Error::DerivedDatasouceOverlappingKeys(s1, s2, sat) => format!("derived table datasources {2:?} have overlapping keys, schemata: {0:?} and {1:?}", sat, s1, s2),
            Error::CannotBeAlgebrized(str) => format!("{0} cannot be algebrized", str),
            Error::SchemaChecking(error) => error.to_string(),
